/// @description Draw debug
	...
	if(show_player && !debug && instance_exists(obj_player))
	{
		...
		draw_set_color(c_blue);
		draw_set_alpha(0.5);
		draw_rectangle(floor(obj_player.x)-obj_player.atk_hitbox_left_offset, floor(obj_player.y)-obj_player.atk_hitbox_top_offset, floor(obj_player.x)+obj_player.atk_hitbox_right_offset, floor(obj_player.y)+ obj_player.atk_hitbox_bottom_offset, false);
		draw_set_alpha(1);
		draw_set_color(c_white);
