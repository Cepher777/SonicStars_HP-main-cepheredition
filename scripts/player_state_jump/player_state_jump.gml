function player_state_jump(){
	...
	var can_jump_states = [ST_NORMAL, ST_ROLL, ST_FORCEROLL, ST_SKID, ST_LOOKUP];
	...
	{
		if(state == ST_NORMAL || state == ST_SPRING || state == ST_FALLING)
		{
			state = ST_JUMP;
			jump_flag = false;
			ceiling_lock = 2;
	...
	if(character != CHAR_TAILS || character != CHAR_AVIA)
	{
